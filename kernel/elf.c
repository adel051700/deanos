...
extern const uint8_t _binary_build_user_fstest_elf_start[];
...
extern const uint8_t _binary_build_user_write_elf_start[];
extern const uint8_t _binary_build_user_write_elf_end[];
extern const uint8_t _binary_build_user_rm_elf_start[];
extern const uint8_t _binary_build_user_rm_elf_end[];
extern const uint8_t _binary_build_user_stat_elf_start[];
extern const uint8_t _binary_build_user_stat_elf_end[];
extern const uint8_t _binary_build_user_id_elf_start[];
...
extern const uint8_t _binary_build_user_time_elf_start[];
...
void elf_install_test_programs(void) {
    ...
    vfs_create(bin, "ls", VFS_FILE);
    vfs_node_t* ls_bin = vfs_finddir(bin, "ls");
    if (!ls_bin) return;
    ...
    vfs_create(bin, "write", VFS_FILE);
    vfs_node_t* write_bin = vfs_finddir(bin, "write");
    if (!write_bin) return;
    uint32_t write_size = (uint32_t)(_binary_build_user_write_elf_end - _binary_build_user_write_elf_start);
    vfs_write(write_bin, 0, write_size, _binary_build_user_write_elf_start);

    vfs_create(bin, "rm", VFS_FILE);
    vfs_node_t* rm_bin = vfs_finddir(bin, "rm");
    if (!rm_bin) return;
    uint32_t rm_size = (uint32_t)(_binary_build_user_rm_elf_end - _binary_build_user_rm_elf_start);
    vfs_write(rm_bin, 0, rm_size, _binary_build_user_rm_elf_start);

    vfs_create(bin, "stat", VFS_FILE);
    vfs_node_t* stat_bin = vfs_finddir(bin, "stat");
    if (!stat_bin) return;
    uint32_t stat_size = (uint32_t)(_binary_build_user_stat_elf_end - _binary_build_user_stat_elf_start);
    vfs_write(stat_bin, 0, stat_size, _binary_build_user_stat_elf_start);

    vfs_create(bin, "id", VFS_FILE);
    vfs_node_t* id_bin = vfs_finddir(bin, "id");
    if (!id_bin) return;
    uint32_t id_size = (uint32_t)(_binary_build_user_id_elf_end - _binary_build_user_id_elf_start);
    vfs_write(id_bin, 0, id_size, _binary_build_user_id_elf_start);

    vfs_create(bin, "pwd", VFS_FILE);
    vfs_node_t* pwd_bin = vfs_finddir(bin, "pwd");
    if (!pwd_bin) return;
    ...
    vfs_create(bin, "time", VFS_FILE);
    vfs_node_t* time_bin = vfs_finddir(bin, "time");
    if (!time_bin) return;
    uint32_t time_size = (uint32_t)(_binary_build_user_time_elf_end - _binary_build_user_time_elf_start);
    vfs_write(time_bin, 0, time_size, _binary_build_user_time_elf_start);

    vfs_create(bin, "tz", VFS_FILE);
    vfs_node_t* tz_bin = vfs_finddir(bin, "tz");
    if (!tz_bin) return;
    ...
    vfs_create(bin, "disk", VFS_FILE);
    vfs_node_t* disk_node = vfs_finddir(bin, "disk");
    if (!disk_node) return;
    ...
    vfs_create(bin, "net", VFS_FILE);
    vfs_node_t* net_node = vfs_finddir(bin, "net");
    if (!net_node) return;
