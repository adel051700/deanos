...
78. Disk-safety guard: when booted via a disk image whose MBR is an isohybrid ISO's boot record (not a deanos filesystem), kernel/mbr.c still registers a partition for it (`mbr: registered partition`), and nothing stops `disk setup`/writes under `/mnt/hdaN` from targeting it — could corrupt the active boot medium. Found during the disk-boot-reliability task (todo 51); boot-from-disk itself works fine, this is a follow-up safety guard, not a boot-reliability bug.
79. ~~Disk install: `disk install <disk> <disk>` writes GRUB + the kernel onto a target disk so it boots standalone, no ISO needed.~~
80. FAT32 mkdir support: `kernel/fat32.c`'s `fat32_create()` currently rejects `VFS_DIRECTORY`, which is why `disk install` writes a flat FAT32-root layout instead of `/boot/grub`; adding directory creation would let install move to the standard layout.
